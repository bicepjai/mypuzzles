...
*/
	...
	public class Solution {
		...
		private static int getFirst( final Comparable<Integer>[] c ) {
		...
		public static void main(String[] args) {
			...
			ArrayList<Integer> permutation = new ArrayList<Integer>(k);
			while(k-- > 0){
			...
			int i = 0, j = 0;
			String yes = "Y";
			Map<Integer,Integer> cond = new HashMap<Integer,Integer>();
			for(i=0;i<K;i++) {
				String word = sc.nextLine();
				int index = word.indexOf(yes);
				while (index >= 0) {
					cond.put(i,index);
					index = word.indexOf(yes, index + 1);
				}
			}
